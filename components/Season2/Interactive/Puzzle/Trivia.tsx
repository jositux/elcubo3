...
const Trivia = ({ question, answers = [], paragraph, onAnswerSelected }) => {
  ...
  return (
    <React.Fragment>
      {showTrivia ?
        <div className={styles.TriviaCover}>
          <div className={styles.Trivia}>
            <h1> {question}</h1>
            {answers && answers.map(({ field_ec_answer, field_ec_text_success, field_ec_score_success }, index) => {
              return <ButtonTrivia onClick={() => handleAnswerSelected(field_ec_text_success, field_ec_score_success)} key={index} text={field_ec_answer} />
            })}
            <p>
              {paragraph}
            </p>
